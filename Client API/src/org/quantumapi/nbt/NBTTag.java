...
public class NBTTag<V extends Number> extends APIObject {
	...
	/**
	 * Formats an NBTBase to an NBTTag<?>. 
	 * 
	 * @param nbt the NBTBase to format
	 * @return the formatted NBTBase
	 * @throws IllegalArgumentException if the given NBTBase is not supported
	 */
	@SuppressWarnings("unchecked")
	public static <V extends Number> NBTTag<V> format(NBTBase nbt) throws IllegalArgumentException {
		switch(nbt.getId()) {
		case 1:
			return (NBTTag<V>) format((NBTTagByte) nbt);
		case 2:
			return (NBTTag<V>) format((NBTTagShort) nbt);
		case 3:
			return (NBTTag<V>) format((NBTTagInt)nbt);
		case 4:
			return (NBTTag<V>) format((NBTTagLong) nbt);
		case 5:
			return (NBTTag<V>) format((NBTTagFloat) nbt);
		case 6:
			return (NBTTag<V>) format((NBTTagDouble)nbt);
		case 7:
		case 8:
		case 11:
			return format(nbt);
			default:
				throw new IllegalArgumentException("Invalid NBTTag type:[" +nbt.getId() + ", \"Unknown\"]");
		}
	}
